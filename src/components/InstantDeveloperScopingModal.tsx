  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/send-enquiry", {
        ...
        body: JSON.stringify({
          formType: "instant-scoping-modal",
          name: fullName,
          email,
          phone,
          company,
      ...
      if (res.ok && data.success) {
        ...
        setErrorMessage(data.error || "Failed to submit developer matching request. Please try again.");
      }
    } catch (err) {
      console.error(err);
          ...
          <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-6 max-h-[80vh] overflow-y-auto bg-white">
            ...
            <div className="pt-4 border-t border-[#E2DDD5]">
              ...
              <div className="pt-2 text-center">
                <Link
                  href={`/build-team/hire?role=${encodeURIComponent(roleTitle)}`}
                  onClick={onClose}
